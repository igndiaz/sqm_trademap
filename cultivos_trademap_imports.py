...
try:
    with open("/home/sqm_repositorio_gcp/app/secrets/trademap_4.json", "r", encoding="UTF-8") as json_file:
...
# intenta acceder a directorio trademap_cultivos_imp, si no existe lo crea
try:
    os.chdir("trademap_cultivos_imp")
except FileNotFoundError:
    os.mkdir("trademap_cultivos_imp")
    print("No existía directorio 'trademap_cultivos_imp', creado nuevo directorio output/trademap_cultivos_imp")
    os.chdir("trademap_cultivos_imp")
except:
    print("Se encontró un error al tratar de entrar a output/trademap_cultivos_imp, no pudo ser creado tampoco. Se muestra error a continuación: \n)")
    raise
...
def divide_fecha(df_):
    ...
    try:
        df["Year_Month"] = df.Year_Month.str.replace("Imported value in ", "")
...
union_no_duplicates.to_csv("trademap_imports_cultivos.csv")
upload_blob("sqm_trademap_cultivos_imports","trademap_imports_cultivos.csv","trademap_imports_cultivos.csv")
...
# Exportación a formato Excel de tabla de buenos:
#union_no_duplicates.to_excel("../output_imports.xlsx")
union_no_duplicates.to_excel("trademap_imports_cultivos.xlsx")
